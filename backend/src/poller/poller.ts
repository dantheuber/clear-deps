...
export async function pollService(serviceId: string, logger: any) {
  ...
  try {
    ...
    try {
      ...
      logger.warn(
        { serviceId: service.id, err: e.message, snippet: text.slice(0, 300) },
    ...
    if (deps.length === 0) {
      if (Array.isArray(json)) {
        ...
        deps = (json as any).dependencies as any;
        parseMode = 'object.dependencies';
    ...
    fastDebug(logger, 'poll.payload.parsed', {
    ...
    const pollRun = await prisma.pollRun.create({
      data: { serviceId: service.id, success: true, httpStatus, durationMs: Date.now() - start },
    });
    ...
    for (const d of deps) {
      ...
      if (
        !depType &&
        checkDetails &&
        typeof checkDetails === 'object' &&
        typeof checkDetails.type === 'string'
      ) {
        depType = checkDetails.type;
      }
      // Check manual override first
      const override = await prisma.dependencyMappingOverride
        .findUnique({
          where: {
            parentServiceId_dependencyName: { parentServiceId: service.id, dependencyName: name },
          },
        })
        .catch(() => null);
      ...
      if (override) {
        ...
        mapped = await prisma.service.findFirst({
          where: { name: name, environment: service.environment },
        });
      }
      let metaStr: string | null = null;
      if ((d as any).meta !== undefined) {
        try {
          metaStr = JSON.stringify((d as any).meta);
        } catch {
          metaStr = null;
        }
      }
      ...
      if (errMsg || errObj) {
        try {
          ...
          if (errMsg && typeof existing.errorMessage !== 'string')
            existing.errorMessage = String(errMsg);
      ...
      normalized.push({
        ...
        dependencyName: name,
        dependencyType: depType || 'unknown',
        mappedServiceId: mapped?.id,
        status,
        metaJson: metaStr,
      });
    }
    if (normalized.length === 0 && deps.length > 0) {
      fastDebug(logger, 'poll.deps.filtered_all_out', {
        serviceId: service.id,
        rawDepsCount: deps.length,
      });
    }
    ...
    if (normalized.length) {
        ...
        await prisma.serviceDependencyCurrent.upsert({
          where: {
            parentServiceId_dependencyName: {
              parentServiceId: service.id,
              dependencyName: n.dependencyName,
            },
          },
          update: {
            dependencyType: n.dependencyType,
            mappedServiceId: n.mappedServiceId,
            status: n.status,
            metaJson: n.metaJson || undefined,
          },
          create: {
            parentServiceId: service.id,
            dependencyName: n.dependencyName,
      ...
      await prisma.serviceDependencyCurrent.deleteMany({
        where: { parentServiceId: service.id, dependencyName: { notIn: names } },
      });
    } else {
    ...
    if (normalized.length) {
      await prisma.dependencyHistory.createMany({
        data: normalized.map((n) => ({
          serviceId: service.id,
          dependencyName: n.dependencyName,
          status: n.status,
    ...
    await prisma.service.update({
      where: { id: service.id },
      data: { lastPolledAt: new Date(), consecutiveFailures: 0 },
    });
    logger.info(
      { serviceId: service.id, serviceName: service.name, deps: normalized.length },
      'poll success'
    );
  } catch (err: any) {
    logger.warn(
      { err: err.message, serviceId: service.id, serviceName: service.name },
      'poll failure'
    );
    const pollRun = await prisma.pollRun.create({
      data: {
